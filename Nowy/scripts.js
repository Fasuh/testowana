...
$(document).ready(function(){
	...
	$("#peleryna").click(function(){
	    ...
	    $("input[name=class]").change(function(){
			stats[0] = $("input[name=class]:checked").val();
			$("div#error1").html("");
			switch (stats[0]) {
				case "1":
					stats[1]="Wojownik";
		...
		$("#containerz").ready(function(){
			if(nick.length!==undefined && nick.length>0 && stats[0] !== undefined){
				stats[7]= $.get("$wiersz['gold']");
				stats[8]=0;
				stats[9]=50;
				stats[10]=1;
				stats[20]=0;
				...
				stats[7]);
				$("div#nick").html(nick);
				$("div#level").html("<br>"+stats[1]+"<br>LvL:"+stats[10]+"<p style='font-size:12px'>Exp:"+stats[8]+"/"+stats[9]+"</p>");
			...
			else {$("div#error1").html("Musisz podac imie postaci oraz wybrac klase!");}
		})
		$("#HP").click(function(){
			stats[20]--
			stats[2]+=15;
